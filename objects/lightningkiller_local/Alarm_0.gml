...
if w_count <= 7
{
	var gem0 = instance_position(x-(w_count*64),y,MyGem)
	var gem1 = instance_position(x+(w_count*64),y,MyGem)
	var gem2 = instance_position(x,y+(w_count*64),MyGem)
	var gem3 = instance_position(x,y-(w_count*64),MyGem)
    if gem0 != noone
	{
		gem[0] = gem0
	}
        ...
        with(gem[i]) 
        {
            if amLit && !amInvisible
            {
				...
				else if gempower !=6
				{
		            ...
		            with(MyGamerule) {points_add_nocombo(20); compliment_add(30) style_add(20,false)}
				...
				else
				{
					if !amFruitExploding
					{
						ds_list_add(MyGamerule.list_of_fruits,other.gem[other.i])
